 ...
 *       addLeft means add a value to the left end, 
 *       addRight means add a value to the right end,
 *       similarly for removeLeft, removeRight
 *               
 ...
 *  The instance variable  left is used to 'point' at the left-most node
 *  The instance variable  right is used to 'point' at the right-most node
 *  The instance variable N keeps a count of the size of the Dequeue
 *  
 *  If you are unable to complete a ToDo, comment out the code you added so the function does not 
 ...
 * 
 * You should not use any loops or recursions, except possibly in "deleteKthFromRight
 * deleteKthFromRight may use one loop or recursive helper.
 ...
 * The testing code for functions besides addRight,addLeft relies on addRight, addLeft being correct.
 * 
...
public class DSIDequeue {
	...
	/** concat
	 * 
	 * The concat method should take the Nodes from "that" and add them to the right hand side of "this"
	 ...
	 * it may not call addLeft or addRight.
	 * preconditions: none ( either dequeue may be empty)
	 * 
	 */
	...
	/** deleteKthFromRight
	 * 
	 * delete and return the kth element from the right hand side (where k is between 0 and N-1).
	 * the 0th node from the right is the rightmost node
	 * See the tests in the main program.
	 *
	 * You may use a loop or a recursive definition here.
	 * This method should create no new Nodes;
	 * it may not call addLeft, addRight
	 * it MAY call removeLeft and removeRight - but think carefully about when these methods would help
	 * 
	 */
	...
	public static void concatTests() {
		...
		d1 = new DSIDequeue ();
		for (int i = 10; i < 15; i++) { d1.addLeft (i); checkInvariants ("left", d1); }
		for (int i = 20; i < 25; i++) { d1.addRight (i); checkInvariants ("right", d1); }
		...
		for (int i = 30; i < 35; i++) { d2.addLeft (i); checkInvariants ("left", d2); }
		for (int i = 40; i < 45; i++) { d2.addRight (i); checkInvariants ("right", d2); }
		...
		check ("concat 7b", d2, "[ ]");
		for (int i = 0; i < 20; i++) { d1.removeLeft (); checkInvariants ("left", d1); }
	...
	public static void deleteKthTests() {
		...
		d1 = new DSIDequeue ();
		for (int i = 10; i < 20; i++) { d1.addRight (i); checkInvariants ("right", d1); }
		k = d1.deleteKthFromRight(0);
